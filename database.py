...
class DB:
    ...
    def add(self, text, audio_len, proc_len):
        self.conn.execute("INSERT INTO history (text, dt, audio_len, proc_len) VALUES (?, datetime('now'), ?, ?)", 
    ...
    def search(self, query):
        q = f"%{query}%"
        return self.conn.execute("SELECT text, dt FROM history WHERE text LIKE ? ORDER BY id DESC", (q,)).fetchall()
